...
const CHARGE_DEFS = [
    { key: "punching", label: "Punching", unit: "₹/unit" },
    { key: "opack", label: "Opack", unit: "₹/unit" },
];

function ChargeCell({ company, chargeDef, handlers, canEdit, readOnly }) {
    const charge = company?.charges?.[chargeDef.key] ?? {
        price: 0,
        isAvailable: true,
    };
    const isAvailable = charge.isAvailable !== false;
    const [price, setPrice] = useState(String(charge.price ?? 0));

    function commit() {
        const next = Number(price);
        if (Number.isNaN(next) || next < 0 || !canEdit || readOnly) return;

        const current = Number(charge.price ?? 0);
        ...
        <div className="py-3 px-4">
            <div className="flex items-center gap-2 min-w-0">
                <IOSToggle
                    on={isAvailable}
                    onToggle={() =>
                        handlers.onToggle(company.id, chargeDef.key, !isAvailable)
                    }
                ...
                >
                    ...
                    />
                    <span className="px-2 text-label-3 text-xs shrink-0">
                        {chargeDef.unit}
                    </span>
                </div>
            </div>
...
export default function FlexoCompanyTableRenderer({
    companies,
    handlers,
    adding,
    onAddStart,
            ...
            <section className="card overflow-hidden min-h-[18rem] flex flex-col">
                {!selectedCompany ? (
                    <div className="h-full flex items-center justify-center text-label-3 text-sm">
                        Select a company to edit charge pricing.
                    </div>
                ) : (
                    <>
                            ...
                            <div>
                                ...
                                <p className="text-xs text-label-3">
                                    {isArchived
                                        ...
                                        : "Punching & Opack pricing and availability controls"}
                                </p>
                            </div>
                        ...
                        <div className="card-section flex-1 min-h-0 overflow-y-auto space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                {CHARGE_DEFS.map((chargeDef) => (
                                    <div
                                        key={`${selectedCompany.id}-${chargeDef.key}`}
